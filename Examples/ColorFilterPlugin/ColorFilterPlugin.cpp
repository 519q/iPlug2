  ...
  : iplug::Plugin(info, MakeConfig(kNumParams, kNumPresets))
{
  ...
  mLayoutFunc = [&](IGraphics* pGraphics) {
    ...
    m_ButtonsPanel = b.GetFromLeft(400.f).GetFromTop(400.f);
    ...
    pGraphics->AttachControl(new IVRadioButtonControl(m_ButtonsPanel.GetGridCell(1, 1, rows, columns).GetFromTop(100).GetMidHPadded(padding), kFilterAlgo, {"DF1", "DF2", "SVF1"}, "FilterAlgo",
...
void ColorFilterPlugin::OnParamChange(int paramIdx, EParamSource, int sampleOffset)
{
  ...
  case kFilterAlgo:
    m_filterAlgo = GetParam(kFilterAlgo)->Int();
    // int currentAlgo = GetParam(kFilterAlgo)->Int();
    // if (m_filterAlgo != currentAlgo)
    //{
    //   m_FilterSelectionMap[m_filterAlgo] = GetParam(kFilterSelector)->Int();
    // }
    // m_filterAlgo = currentAlgo;
    int min_value{};
    int max_value{};
