...
def  main():
  ...
  installer = "\installer\ColorFilterPlugin Installer.exe"
   
  if demo:
    installer = "\installer\ColorFilterPlugin Demo Installer.exe"
   
  FILES_TO_ZIP = [
    projectpath + installer,
    projectpath + "\installer\changelog.txt",
    projectpath + "\installer\known-issues.txt",
    projectpath + "\manual\ColorFilterPlugin manual.pdf" 
  ]
  ...
  ZIPNAME = "ColorFilterPlugin-v" + FULLVERSIONSTR + "-win.zip"
  
  if demo:
    ZIPNAME = "ColorFilterPlugin-v" + FULLVERSIONSTR + "-win-demo.zip"
