 ...
 *   @ingroup APIClasses */
class IPlugCLAP : public IPlugAPIBase, public IPlugProcessor, public ClapPluginHelper
{
  struct ParamToHost
  {
    enum class Type
...
private:
  ...
  // Buffer for parameter modulation
  std::unordered_map<int, std::unordered_map<int, double>> PolyphonicModulationBuffer; // Param ID -> [Modulation per note]
  std::unordered_map<int, double> MonophonicModulationBuffer;                          // Param ID -> single modulation value (for monophonic)
};
