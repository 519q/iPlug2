...
uint32_t IPlugCLAP::tailGet() const noexcept { return GetTailIsInfinite() ? std::numeric_limits<uint32_t>::max() : GetTailSize(); }

void IPlugCLAP::BeginInformHostOfParamChange(int idx) { mParamValuesToHost.PushFromArgs(ParamToHost::Type::Begin, idx, 0.0); }
...
void IPlugCLAP::EndInformHostOfParamChange(int idx) { mParamValuesToHost.PushFromArgs(ParamToHost::Type::End, idx, 0.0); }
...
bool IPlugCLAP::paramsInfo(uint32_t paramIdx, clap_param_info* pInfo) const noexcept
{
  ...
  // checks for flag in initializer and sends it to host
  if (pParam->GetFlags() & CLAP_PARAM_IS_MODULATABLE)
  ...
  if (pInputEvents)
  {
    for (int i = 0; i < pInputEvents->size(pInputEvents); i++)
    {
      ...
      case CLAP_EVENT_PARAM_MOD: {
        // Handle parameter modulation event
        auto pParamMod = ClapEventCast<clap_event_param_mod>(pEvent);
        int paramIdx = pParamMod->param_id;
        double modAmount = pParamMod->amount;
        bool isPolyphonic = pParamMod->note_id >= 0; // Check if note_id is present (polyphonic)
        // Retrieve the parameter and apply modulation
        IParam* pParam = GetParam(paramIdx);
        if (pParam)
          if (isPolyphonic)
          {
            // Polyphonic modulation: Store modulation per note
            // Use the note_id to index the modulation buffer
            PolyphonicModulationBuffer[paramIdx][pParamMod->note_id] = modAmount;
          }
          else
          {
            // Monophonic modulation: Store single modulation value
            MonophonicModulationBuffer[paramIdx] = modAmount;
          }
        break;
      }
      default:
...
bool IPlugCLAP::implementsAudioPorts() const noexcept { return MaxNBuses(ERoute::kInput) || MaxNBuses(ERoute::kOutput); }

uint32_t IPlugCLAP::audioPortsCount(bool isInput) const noexcept { return NBuses(isInput ? ERoute::kInput : ERoute::kOutput); }
...
bool IPlugCLAP::audioPortsInfo(uint32_t index, bool isInput, clap_audio_port_info* pInfo) const noexcept
{
  ...
  constexpr uint32_t bitFlags = CLAP_AUDIO_PORT_SUPPORTS_64BITS | CLAP_AUDIO_PORT_PREFERS_64BITS | CLAP_AUDIO_PORT_REQUIRES_COMMON_SAMPLE_SIZE;
...
bool IPlugCLAP::implementsAudioPortsConfig() const noexcept { return audioPortsConfigCount(); }

uint32_t IPlugCLAP::audioPortsConfigCount() const noexcept { return static_cast<uint32_t>(NIOConfigs()); }
...
bool IPlugCLAP::audioPortsGetConfig(uint32_t index, clap_audio_ports_config* pConfig) const noexcept
{
  ...
  auto getNChans = [&](ERoute direction, int bus) { return static_cast<uint32_t>(NChannels(direction, static_cast<uint32_t>(bus), index)); };
...
bool IPlugCLAP::implementsGui() const noexcept { return HasUI(); }

bool IPlugCLAP::guiCreate(const char* api, bool isFloating) noexcept { return HasUI(); }
...
bool IPlugCLAP::guiCanResize() const noexcept { return HasUI() && GetHostResizeEnabled(); }
...
int IPlugCLAP::RequiredChannels() const { return std::max(MaxNChannels(kInput), MaxNChannels(kOutput)); }

uint32_t IPlugCLAP::NBuses(ERoute direction, int configIdx) const { return GetIOConfig(configIdx)->NBuses(direction); }

uint32_t IPlugCLAP::NChannels(ERoute direction, uint32_t bus, int configIdx) const { return GetIOConfig(configIdx)->NChansOnBusSAFE(direction, static_cast<int>(bus)); }

uint32_t IPlugCLAP::NBuses(ERoute direction) const { return NBuses(direction, mConfigIdx); }

uint32_t IPlugCLAP::NChannels(ERoute direction, uint32_t bus) const { return NChannels(direction, bus, mConfigIdx); }

double IPlugCLAP::GetModulatedParamOffset(int paramIdx, int noteID)
{
  IParam* pParam = GetParam(paramIdx);
  if (!pParam)
    return 0.0;
  ...
  // Check for polyphonic modulation first
  if (noteID >= 0 && PolyphonicModulationBuffer[paramIdx].count(noteID))
  {
    modValue = PolyphonicModulationBuffer[paramIdx][noteID]; // Get modulation for this note
  }
  else if (MonophonicModulationBuffer.count(paramIdx))
  {
    modValue = MonophonicModulationBuffer[paramIdx]; // Otherwise, check monophonic modulation
  }
  return modValue;
}
