...
inline float BlendWeight(const IBlend* pBlend) { return (pBlend ? pBlend->mWeight : 1.0f); }
...
struct IRECT
{
   ...
   * @param h Height of new IRECT
   * @return the new IRECT */
  static IRECT MakeMidXYWH(float x, float y, float w, float h) { return IRECT(x - (w / 2.0), y - (h / 2.0), x + (w / 2.0), y + (h / 2.0)); }
  ...
  inline bool Intersects(const IRECT& rhs) const { return (!Empty() && !rhs.Empty() && R >= rhs.L && L < rhs.R && B >= rhs.T && T < rhs.B); }

  /** Returns true if this IRECT completely contains `rhs`.
   * @param rhs another IRECT
   * @return true if this IRECT completely contains `rhs`
   * @return false if any part of `rhs` is outside this IRECT */
  inline bool Contains(const IRECT& rhs) const { return (!Empty() && !rhs.Empty() && rhs.L >= L && rhs.R <= R && rhs.T >= T && rhs.B <= B); }
   ...
   * @param padB Bottom-padding
   * @return IRECT the new rectangle */
  inline IRECT GetPadded(float padL, float padT, float padR, float padB) const { return IRECT(L - padL, T - padT, R + padR, B + padB); }
...
public:
  ...
  static bool GetFracGrid(
    const IRECT& input, IRECTList& rects, const std::initializer_list<float>& rowFractions, const std::initializer_list<float>& colFractions, EDirection layoutDir = EDirection::Horizontal)
  {
...
struct IMatrix
{
   ...
   * @return IMatrix& The matrix */
  IMatrix& Skew(float xa, float ya) { return Transform(IMatrix(1.0, std::tan(DegToRad(ya)), std::tan(DegToRad(xa)), 1.0, 0.0, 0.0)); }
...
struct IPattern
{
  ...
  ////default constructor for IPattern
  ...
  static IPattern CreateLinearGradient(float x1, float y1, float x2, float y2, const std::initializer_list<IColorStop>& stops = {})
  {
    ...
    pattern.SetTransform(static_cast<float>(c), static_cast<float>(s), static_cast<float>(-s), static_cast<float>(c), static_cast<float>(x0), static_cast<float>(y0));
   ...
   * @param tx X translation component of the affine transformation
   * @param ty Y translation component of the affine transformation */
  void SetTransform(float xx, float yx, float xy, float yy, float tx, float ty) { mTransform = IMatrix(xx, yx, xy, yy, tx, ty); }
...
static const IPattern DEFAULT_BGCOLOR_PATTERN = IPattern(DEFAULT_BGCOLOR);
static const IPattern DEFAULT_FGCOLOR_PATTERN = IPattern(DEFAULT_FGCOLOR);
static const IPattern DEFAULT_PRCOLOR_PATTERN = IPattern(DEFAULT_PRCOLOR);
static const IPattern DEFAULT_FRCOLOR_PATTERN = IPattern(DEFAULT_FRCOLOR);
static const IPattern DEFAULT_HLCOLOR_PATTERN = IPattern(DEFAULT_HLCOLOR);
static const IPattern DEFAULT_SHCOLOR_PATTERN = IPattern(DEFAULT_SHCOLOR);
static const IPattern DEFAULT_X1COLOR_PATTERN = IPattern(DEFAULT_X1COLOR);
static const IPattern DEFAULT_X2COLOR_PATTERN = IPattern(DEFAULT_X2COLOR);
static const IPattern DEFAULT_X3COLOR_PATTERN = IPattern(DEFAULT_X3COLOR);
...
struct IVPatternSpec
{
  ...
  const IPattern& GetPattern(EVColor color) const { return mPatterns[(int)color]; }

  /** @return The default IPattern for an EVColor */
  static const IPattern& GetDefaultPattern(EVColor idx)
  {
    switch (idx)
    {
    case kBG:
      return DEFAULT_BGCOLOR_PATTERN;
    case kFG:
      return DEFAULT_FGCOLOR_PATTERN;
    case kPR:
      return DEFAULT_PRCOLOR_PATTERN;
    case kFR:
      return DEFAULT_FRCOLOR_PATTERN;
    case kHL:
      return DEFAULT_HLCOLOR_PATTERN;
    case kSH:
      return DEFAULT_SHCOLOR_PATTERN;
    case kX1:
      return DEFAULT_X1COLOR_PATTERN;
    case kX2:
      return DEFAULT_X2COLOR_PATTERN;
    case kX3:
      return DEFAULT_X3COLOR_PATTERN;
    default:
      return IPattern(COLOR_TRANSPARENT);
    };
  }

  ///** Create a new IVPatternSpec object with default colors */
  IVPatternSpec()
  {
    for (int i = 0; i < kNumVColors; i++)
    {
      mPatterns[i] = GetDefaultPattern((EVColor)i); // Explicitly initialize each pattern
    }
  }

  /** Create a new IVColorSpec object specifying the colors
   * @param colors Initializer list of IColors */
  IVPatternSpec(const std::initializer_list<IPattern>& patterns)
  {
    assert(patterns.size() <= kNumVColors);
    ...
    for (; i < kNumVColors; i++)
    {
      mPatterns[i] = GetDefaultPattern((EVColor)i);
    }
  }
