...
class IVLabelControl : public ITextControl, public IVectorBase
{
public:
  IVLabelControl(const IRECT& bounds,
                 const char* label,
                 const IVStyle& style = DEFAULT_STYLE.WithDrawFrame(false).WithColor(kSH, COLOR_BLACK).WithShadowOffset(1).WithValueText(DEFAULT_VALUE_TEXT.WithSize(20.f).WithFGColor(COLOR_WHITE)));
  void Draw(IGraphics& g) override;
};

/** A vector button/momentary switch control. */
class IVButtonControl : public IButtonControlBase, public IVectorBase
{
public:
  ...
  IVButtonControl(const IRECT& bounds,
...
class IVSwitchControl : public ISwitchControlBase, public IVectorBase
{
public:
...
public:
  ...
  IVToggleControl(const IRECT& bounds,
                  IActionFunction aF = SplashClickActionFunc,
                  const char* label = "",
                  const IVStyle& style = DEFAULT_STYLE,
...
public:
  ...
  IVSlideSwitchControl(
    const IRECT& bounds, int paramIdx = kNoParameter, const char* label = "", const IVStyle& style = DEFAULT_STYLE, bool valueInButton = false, EDirection direction = EDirection::Horizontal);
  ...
  IVSlideSwitchControl(const IRECT& bounds,
...
class IVTabSwitchControl : public ISwitchControlBase, public IVectorBase
{
public:
   ...
   * @param direction The direction of the buttons */
  IVTabSwitchControl(const IRECT& bounds,
...
public:
  ...
  IVRadioButtonControl(const IRECT& bounds,
                       int paramIdx = kNoParameter,
...
class IVMenuButtonControl : public IContainerBase, public IVectorBase
{
public:
  /** Constructs a vector button control, with an action function
   * @param bounds The control's bounds
...
{
public:
  IVKnobControl(const IRECT& bounds,
                int paramIdx,
                const char* label = "",
                const IVStyle& style = DEFAULT_STYLE,
...
class IVSliderControl : public ISliderControlBase, public IVectorBase
{
public:
  IVSliderControl(const IRECT& bounds,
                  int paramIdx = kNoParameter,
                  const char* label = "",
                  const IVStyle& style = DEFAULT_STYLE,
                  bool valueIsEditable = false,
  ...
  IRECT GetTrackBounds() const
  {
    ...
    return mWidgetBounds.GetPadded(mDirection == EDirection::Horizontal ? offset : 0, mDirection == EDirection::Vertical ? offset : 0, mDirection == EDirection::Horizontal ? offset : 0,
...
public:
  ...
  IVPlotControl(
    const IRECT& bounds, const std::initializer_list<Plot>& funcs, int numPoints, const char* label = "", const IVStyle& style = DEFAULT_STYLE, float min = -1., float max = 1., bool useLayer = false);
...
class IVGroupControl : public IContainerBase, public IVectorBase
{
public:
  ...
  IVGroupControl(const IRECT& bounds,
                 const char* label = "",
...
class IVPanelControl : public IContainerBase, public IVectorBase
{
public:
  ...
  void DrawWidget(IGraphics& g) override { DrawPressableRectangle(g, mWidgetBounds, false, false, false); }
...
public:
  ...
  IVColorSwatchControl(const IRECT& bounds,
                       const char* label = "",
                       ...
                       const std::initializer_list<const char*>& labelsForIDs = {
                         kVColorStrs[kBG], kVColorStrs[kFG], kVColorStrs[kPR], kVColorStrs[kFR], kVColorStrs[kHL], kVColorStrs[kSH], kVColorStrs[kX1], kVColorStrs[kX2], kVColorStrs[kX3]});
...
public:
  ...
  ISVGButtonControl(const IRECT& bounds,
                    IActionFunction aF,
                    const ISVG& image,
...
public:
   ...
   * @param colors Colors to replace the SVG's fill/stroke in the off/on/mouse-over-off/mouse-over-on states
   * @param colorReplacement Should the fill or stroke in the SVG be colored */
  ISVGToggleControl(const IRECT& bounds,
                    IActionFunction aF,
                    const ISVG& image,
                    const std::array<IColor, 4> colors = {COLOR_BLACK, COLOR_WHITE, COLOR_DARK_GRAY, COLOR_LIGHT_GRAY},
                    EColorReplacement colorReplacement = EColorReplacement::Fill);
...
class IBButtonControl : public IButtonControlBase, public IBitmapBase
{
public:
...
class IBSwitchControl : public ISwitchControlBase, public IBitmapBase
{
public:
...
class IBKnobControl : public IKnobControlBase, public IBitmapBase
{
public:
...
class IBSliderControl : public ISliderControlBase, public IBitmapBase
{
public:
  IBSliderControl(float x,
                  float y,
                  float trackLength,
  ...
  IBSliderControl(
    const IRECT& bounds, const IBitmap& handleBitmap, const IBitmap& trackBitmap = IBitmap(), int paramIdx = kNoParameter, EDirection dir = EDirection::Vertical, double gearing = DEFAULT_GEARING);
...
class IBTextControl : public ITextControl, public IBitmapBase
{
public:
  IBTextControl(const IRECT& bounds,
