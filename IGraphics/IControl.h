...
public:
  ...
  /** Called after the control has been attached, and its delegate and graphics member variable set. Use this method for controls that might need to attach sub controls that should be above their
   * parent in the stack */
  ...
  /** Called by default when the user right clicks a control. If IGRAPHICS_NO_CONTEXT_MENU is enabled as a preprocessor macro right clicking control will mean IControl::CreateContextMenu() and
   * IControl::OnContextSelection() do not function on right clicking control. VST3 provides contextual menu support which is hard wired to right click controls by default. You can add custom items to
   * the menu by implementing IControl::CreateContextMenu() and handle them in IControl::OnContextSelection(). In non-VST 3 hosts right clicking will still create the menu, but it will not feature
   * entries added by the host. */
  ...
  inline IControl* SetActionFunction(IActionFunction actionFunc)
  {
   ...
   * @param actionFunc A std::function conforming to IActionFunction
   * @return Ptr to this control, for chaining */
  inline IControl* SetAnimationEndActionFunction(IActionFunction actionFunc)
  {
    mAnimationEndActionFunc = actionFunc;
    return this;
  }
   ...
   * @return Ptr to this control, for chaining */
  inline IControl* SetTooltip(const char* str)
  {
    mTooltip.Set(str);
    return this;
  }
   ...
   * If you are calling this "manually" to reuse a control for multiple parameters, you probably want to call IEditorDelegate::SendCurrentParamValuesFromDelegate() afterward, to update the control
   * values
  ...
  /** Set the size of the control, preserving the current position. This may need to be overriden if you maintain custom positioning data in your control, or if your TargetRECT is not the same as the
   * main RECT.
  ...
  /** This can be used in IControl::Draw() to check if the mouse is over the control, without implementing mouse over methods. Note that this method is only enabled if IGraphics::EnableMouseOver() is
   * set to \c true
...
public:
  ...
  void SetAttachFunc(AttachFunc attachFunc) { mAttachFunc = attachFunc; }

  void SetResizeFunc(ResizeFunc resizeFunc) { mResizeFunc = resizeFunc; }

  virtual void Draw(IGraphics& g) override { /* NO-OP */ }

  virtual ~IContainerBase() { mChildren.Empty(false); }
  ...
  IControl* GetChild(int idx) { return mChildren.Get(idx); }
...
public:
  ...
  void SetColors(const IVColorSpec& spec) { mStyle.colorSpec = spec; }

  /** Get value of a specific EVColor in the IVControl */
  const IColor& GetColor(EVColor color) const { return mStyle.colorSpec.GetColor(color); }

  /** Get value of a specific (pattern)EVColor in the IVControl */
  const IPattern& GetPattern(EVColor color) const { return mStyle.patternSpec.GetPattern(color); }
  ...
  {
    mStyle.drawFrame = draw;
    mControl->SetDirty(false);
    OnStyleChanged();
  }
  void SetDrawShadows(bool draw)
  {
    mStyle.drawShadows = draw;
    mControl->SetDirty(false);
    OnStyleChanged();
  }
  ...
  {
    mStyle.shadowOffset = offset;
    mControl->SetDirty(false);
    OnStyleChanged();
  }
  void SetFrameThickness(float thickness)
  {
    mStyle.frameThickness = thickness;
    mControl->SetDirty(false);
    OnStyleChanged();
  }
  void SetSplashRadius(float radius)
  {
    mSplashRadius = radius * mMaxSplashRadius;
    OnStyleChanged();
  }
  ...
  void DrawPressableEllipse(IGraphics& g, const IRECT& bounds, bool pressed, bool mouseOver, bool disabled)
  {
    ...
    if (!pressed && !disabled && mStyle.drawShadows)
      g.FillEllipse(mStyle.usingGradients ? GetPattern(kSH) : GetColor(kSH), shadowBounds);
    ...
    if (pressed)
    {
      if (mStyle.emboss)
      {
        ...
        g.FillEllipse(mStyle.usingGradients ? GetPattern(kPR) : GetColor(kPR), bounds, &blend);
        g.FillEllipse(mStyle.usingGradients ? GetPattern(kSH) : GetColor(kSH), bounds, &blend);

        // Inverse shading for recessed look - shadowBounds = inner shadow
        g.FillEllipse(mStyle.usingGradients ? GetPattern(kFG) : GetColor(kFG).WithContrast(contrast), shadowBounds /*, &blend*/);

        // Fill in center with pressed color
        g.FillEllipse(mStyle.usingGradients ? GetPattern(kPR) : GetColor(kPR).WithContrast(contrast), centreBounds /*, &blend*/);
      }
      else
        g.FillEllipse(mStyle.usingGradients ? GetPattern(kPR) : GetColor(kPR).WithContrast(contrast), handleBounds /*, &blend*/);
    }
    else
    {
      ...
      if (mStyle.emboss)
      {
        ...
        g.FillEllipse(mStyle.usingGradients ? GetPattern(kPR) : GetColor(kPR).WithContrast(contrast), bounds /*, &blend*/);

        // Negative light TODO: clip this?
        g.FillEllipse(mStyle.usingGradients ? GetPattern(kSH) : GetColor(kSH).WithContrast(contrast), shadowBounds /*, &blend*/);

        // Fill in foreground
        g.FillEllipse(mStyle.usingGradients ? GetPattern(kFG) : GetColor(kFG).WithContrast(contrast), centreBounds /*, &blend*/);

        // Shade when hovered
        if (mouseOver)
          g.FillEllipse(mStyle.usingGradients ? GetPattern(kHL) : GetColor(kHL), centreBounds, &blend);
      }
      else
      {
        g.FillEllipse(mStyle.usingGradients ? GetPattern(kFG) : GetColor(kFG).WithContrast(contrast), handleBounds /*, &blend*/);

        // Shade when hovered
        if (mouseOver)
          g.FillEllipse(mStyle.usingGradients ? GetPattern(kHL) : GetColor(kHL), handleBounds, &blend);
      }
    }
  ...
  IRECT DrawPressableRectangle(
    IGraphics& g, const IRECT& bounds, bool pressed, bool mouseOver, bool disabled, bool rtl = true, bool rtr = true, bool rbl = true, bool rbr = true, bool usingGradients = false)
  {
    ...
    if (pressed)
    {
      ...
      {
        // Fill background with pressed color and shade it
        g.FillRoundRect(mStyle.usingGradients ? GetPattern(kPR) : GetColor(kPR), handleBounds, tlr, trr, blr, brr, &blend);
        g.FillRoundRect(mStyle.usingGradients ? GetPattern(kSH) : GetColor(kSH), handleBounds, tlr, trr, blr, brr, &blend);

        // Inverse shading for recessed look - shadowBounds = inner shadow
        g.FillRoundRect(mStyle.usingGradients ? GetPattern(kFG) : GetColor(kFG).WithContrast(contrast), shadowBounds, tlr, trr, blr, brr /*, &blend*/);

        // Fill in center with pressed color
        g.FillRoundRect(mStyle.usingGradients ? GetPattern(kPR) : GetColor(kPR), centreBounds, tlr, trr, blr, brr, &blend);
      }
      else
      {
        g.FillRoundRect(mStyle.usingGradients ? GetPattern(kPR) : GetColor(kPR).WithContrast(contrast), handleBounds, tlr, trr, blr, brr /*, &blend*/);
      }
    }
    else
    {
      // outer shadow
      if (mStyle.drawShadows)
        g.FillRoundRect(mStyle.usingGradients ? GetPattern(kSH) : GetColor(kSH), shadowBounds, tlr, trr, blr, brr, &blend);
      ...
      if (mStyle.emboss)
      {
        ...
        // Negative light TODO: clip this?
        g.FillRoundRect(mStyle.usingGradients ? GetPattern(kSH) : GetColor(kSH).WithContrast(contrast), shadowBounds, tlr, trr, blr, brr /*, &blend*/);

        // Fill in foreground
        g.FillRoundRect(mStyle.usingGradients ? GetPattern(kFG) : GetColor(kFG).WithContrast(contrast), centreBounds, tlr, trr, blr, brr /*, &blend*/);

        // Shade when hovered
        if (mouseOver)
          g.FillRoundRect(mStyle.usingGradients ? GetPattern(kHL) : GetColor(kHL), centreBounds, tlr, trr, blr, brr, &blend);
      }
      else
      {
        g.FillRoundRect(mStyle.usingGradients ? GetPattern(kFG) : GetColor(kFG).WithContrast(contrast), handleBounds, tlr, trr, blr, brr /*, &blend*/);

        // Shade when hovered
        if (mouseOver)
          g.FillRoundRect(mStyle.usingGradients ? GetPattern(kHL) : GetColor(kHL), handleBounds, tlr, trr, blr, brr, &blend);
      }
    }
  ...
  IRECT DrawPressableTriangle(IGraphics& g, const IRECT& bounds, bool pressed, bool mouseOver, float angle, bool disabled, bool useGradients = false)
  {
    ...
    if (pressed)
      g.FillTriangle(/*useGradients ? GetPattern(kPR) : */ GetColor(kPR).WithContrast(contrast), x1, y1, x2, y2, x3, y3 /*, &blend*/);
    else
    {
      // outer shadow
      if (mStyle.drawShadows)
        g.FillTriangle(/*useGradients ? GetPattern(kSH) : */ GetColor(kSH), x1 + mStyle.shadowOffset, y1 + mStyle.shadowOffset, x2 + mStyle.shadowOffset, y2 + mStyle.shadowOffset,
                       x3 + mStyle.shadowOffset, y3 + mStyle.shadowOffset, &blend);

      g.FillTriangle(/*useGradients ? GetPattern(kFG) : */ GetColor(kFG).WithContrast(contrast), x1, y1, x2, y2, x3, y3 /*, &blend*/);
    }

    if (mouseOver)
      g.FillTriangle(/*useGradients ? GetPattern(kHL) : */ GetColor(kHL), x1, y1, x2, y2, x3, y3, &blend);
...
/** A base class for controls that can do multitouch */
...
public:
  ...
  int NTrackedTouches() const { return static_cast<int>(mTrackedTouches.size()); }

  TrackedTouch* GetTouch(int index)
  {
    auto itr = std::find_if(mTrackedTouches.begin(), mTrackedTouches.end(), [index](auto element) { return (element.second.index == index); });
...
protected:
  /** Get the area for which mouse deltas will be used to calculate the amount dragging changes the control value. This is usually the area that contains the knob handle, can override if your control
   * contains extra elements such as labels
...
class IVTrackControlBase : public IControl, public IVectorBase
{
public:
  IVTrackControlBase(
    const IRECT& bounds, const char* label, const IVStyle& style, int maxNTracks = 1, int nSteps = 0, EDirection dir = EDirection::Horizontal, std::initializer_list<const char*> trackNames = {})
    : IControl(bounds)
    , IVectorBase(style)
    , mDirection(dir)
    , mNSteps(nSteps)
  ...
  IVTrackControlBase(const IRECT& bounds,
                     const char* label,
                     const IVStyle& style,
                     int lowParamidx,
                     int maxNTracks = 1,
  ...
  virtual ~IVTrackControlBase() { mTrackNames.Empty(true); }
  ...
  bool HasTrackNames() const { return mTrackNames.GetSize() > 0; }
...
protected:
  ...
  virtual void DrawTrack(IGraphics& g, const IRECT& r, int chIdx)
  {
    ...
    if (bv > 0.f)
    {
      ...
      {
        fillRect = IRECT(r.L, trackPos < bv ? r.B - r.H() * bv : r.B - r.H() * trackPos, r.R, trackPos < bv ? r.B - r.H() * trackPos : r.B - r.H() * bv);
      }
      else
      {
        fillRect = IRECT(trackPos < bv ? r.L + r.W() * trackPos : r.L + r.W() * bv, r.T, trackPos < bv ? r.L + r.W() * bv : r.L + r.W() * trackPos, r.B);
      }
    }
    else
    {
    ...
    else
    {
      ...
      if (mDirection == EDirection::Vertical)
      {
        peakRect = IRECT(fillRect.L, trackPos < mBaseValue ? fillRect.B : fillRect.T, fillRect.R, trackPos < mBaseValue ? fillRect.B - mPeakSize : fillRect.T + mPeakSize);
      }
      else
      {
        peakRect = IRECT(trackPos < mBaseValue ? fillRect.L + mPeakSize : fillRect.R - mPeakSize, fillRect.T, trackPos < mBaseValue ? fillRect.L : fillRect.R, fillRect.B);
      }
  ...
  virtual void DrawTrackBackground(IGraphics& g, const IRECT& r, int chIdx) { g.FillRect(chIdx == mHighlightedTrack ? this->GetColor(kHL) : COLOR_TRANSPARENT, r); }

  virtual void DrawTrackName(IGraphics& g, const IRECT& r, int chIdx) { g.DrawText(mText, GetTrackName(chIdx), r); }
  ...
  virtual void DrawPeak(IGraphics& g, const IRECT& r, int chIdx, bool aboveBaseValue) { g.FillRect(GetColor(kFR), r, &mBlend); }
  ...
  virtual void MakeTrackRects(const IRECT& bounds)
  {
    ...
    for (int ch = 0; ch < nVals; ch++)
    {
      mTrackBounds.Get()[ch] = bounds.SubRect(EDirection(!dir), nVals, ch).GetPadded(0, -mTrackPadding * (float)dir, -mTrackPadding * (float)!dir, -mTrackPadding);
    }
  }
  ...
  bool GetStepped() const { return mStepBounds.GetSize() > 0; }
...
{
public:
  IPanelControl(const IRECT& bounds, const IColor& color, bool drawFrame = false, AttachFunc attachFunc = nullptr, ResizeFunc resizeFunc = nullptr)
  ...
  IPanelControl(const IRECT& bounds, const IPattern& pattern, bool drawFrame = false, AttachFunc attachFunc = nullptr, ResizeFunc resizeFunc = nullptr)
    : IContainerBase(bounds, attachFunc, resizeFunc)
    , mPattern(pattern)
    , mDrawFrame(drawFrame)
  {
...
{
public:
  ILambdaControl(const IRECT& bounds,
                 ILambdaDrawFunction drawFunc,
...
class IBitmapControl : public IControl, public IBitmapBase
{
public:
...
public:
  ...
  void OnMouseDown(float x, float y, const IMouseMod& mod) override { GetUI()->CreateTextEntry(*this, mText, mRECT, GetStr()); }
...
{
public:
  IURLControl(const IRECT& bounds,
              const char* str,
              const char* url,
...
public:
  ITextToggleControl(
    const IRECT& bounds, int paramIdx = kNoParameter, const char* offText = "OFF", const char* onText = "ON", const IText& text = DEFAULT_TEXT, const IColor& BGColor = DEFAULT_BGCOLOR);

  ITextToggleControl(
    const IRECT& bounds, IActionFunction aF = nullptr, const char* offText = "OFF", const char* onText = "ON", const IText& text = DEFAULT_TEXT, const IColor& BGColor = DEFAULT_BGCOLOR);
